...
For every existing server, INSERT a role with `id = server_id`, `name = '@everyone'`, `position = 0`, `permissions = DEFAULT_EVERYONE_PERMISSIONS`. The `CreateServer` flow is updated to do this automatically for new servers.
...
```json
{ "permissions": 131071, "is_owner": false }
```
...
Requires `ManageRoles` permission. Can only set permission bits the actor has. Server-only bits (0–13) are masked out.
...
Returns `{ "permissions": 131071 }` with all overwrites applied.
...
3. Replace their overwrites with the category's current full overwrite set
