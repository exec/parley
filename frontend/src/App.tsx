...
function MainApp() {
  ...
  if (showDiscovery) {
    ...
    if (inThisDmCall && currentUser) {
      // In-call layout: voice on top, text chat on bottom, draggable resizer
      // between. Either pane can be hidden (chat snaps closed below threshold).
      ...
      mainContent = (
        <SplitVoiceChat
          voice={voicePanel}
          chat={chatPanel}
          storageKey={`dm:${activeDmChannel.id}`}
        />
      );
    } else {
