  ...
  const receiveUserUpdate = useCallback((update: UserUpdate) => {
    ...
    setMessages(prev => prev.map(m =>
      m.author_id === update.user_id
        ? { ...m, author_username: update.username, author_avatar_url: update.avatar_url ?? m.author_avatar_url, author_display_name: update.display_name ?? m.author_display_name }
        : m
    ));
  }, []);
